package com.Vendedores.bibliotecaduoc.service;

import com.ejemplo.vendedores.entity.Vendedor;
import com.ejemplo.vendedores.repository.VendedorRepository;
import com.ejemplo.vendedores.repository.SucursalAsignadaRepository; // Importar
...
public class VendedorService {
    ...
    public Optional<Vendedor> actualizarVendedor(Long id, Vendedor vendedorActualizado) {
        return vendedorRepository.findById(id)
            .map(vendedorExistente -> {
                ...
                // Manejar la actualización de listas (sucursales, metas) puede ser más complejo
                // y podrías necesitar lógica específica para añadir, eliminar o modificar elementos
                // de las colecciones.
    ...
    public List<Vendedor> obtenerVendedoresPorSucursal(Long idSucursal) {
        ...
        // Alternativa si quisieras más control o lógica aquí:
        // List<SucursalAsignada> asignaciones = sucursalAsignadaRepository.findByIdSucursal(idSucursal);
        // return asignaciones.stream().map(SucursalAsignada::getVendedor).distinct().collect(Collectors.toList());
    }
